...
[MetaData("android.appwidget.provider", Resource = "@xml/checkin_widget_info")]
public class CheckInWidgetProvider : AppWidgetProvider
{
    ...
    {
        var views = new RemoteViews(context.PackageName ?? "", Resource.Layout.checkin_widget);
        ...
        if (selectedProject != null)
        {
            ...
            views.SetTextViewText(Resource.Id.widget_checkin_count, $"已打卡: {selectedProject.CheckInTimes.Count} 次");
            
            var checkInIntent = new Intent(context, typeof(CheckInWidgetProvider));
        ...
        else
        {
            ...
            views.SetTextViewText(Resource.Id.widget_checkin_count, "请先添加项目");
        }
        
        var refreshIntent = new Intent(context, typeof(CheckInWidgetProvider));
        refreshIntent.SetAction(ActionRefresh);
        refreshIntent.PutExtra("appWidgetId", appWidgetId);
        var refreshPendingIntent = PendingIntent.GetBroadcast(context, appWidgetId + 1000, refreshIntent, PendingIntentFlags.UpdateCurrent | PendingIntentFlags.Immutable);
        views.SetOnClickPendingIntent(Resource.Id.widget_title, refreshPendingIntent);
